...
经过比较，发现C的影响没有σ的影响大。而且对于3600规模的数据集，最好的σ取值在1.3，而此时数据的平均距离为45.26，因此最佳放缩为 0.03.
...
- 按照交叉验证，使用3600的数据量，在C=10的情况下，得到的Micro F1和σ取值的关系图如下：
  ![ss](pics/SVM-rel.png)  
  根据关系图，预测最佳sigma值为1.3。

- 按照最佳参数，C=10，σ=1.3, 在减半的训练-测试集上运行（10000训练集大小, 2500测试集大小），得到的最终评估结果如下：
