...
| KNN(欧式距离) | 处理(4个) | 100% |  0.322046 | 0.228962 | 0.322046 |
| KNN(曼哈顿距离) | 处理(4个) | 100% |  0.322402 | 0.229786 | 0.322402 |
...
- 可以调节的参数以及计划的调节范围是：
  - σ： 计划调节：平均距离, 平均距离/10~平均距离/40, 平均距离/100。重点是平均距离/10~平均距离/40的区间。
  - C:  即算法中的marginC，计划调节10，20，1000
- 由于交叉验证很慢，我使用3600个元素的训练集先进行大规模交叉验证。找到最佳参数后，再用稍大一些的规模尝试最终结果。所有的参数组合和交叉验证结果在下表。

- 我使用的数据量都是3600大小的训练集。首先用数据的平均距离公式算出平均距离为45.26。  
  - 选择σ为0.4526，0.9052，1，1.6，2，4.526，45.26. 表格如下；
...
- 经过比较，发现C的影响没有σ的影响大。而且对于3600规模的数据集，最好的σ取值在1.0~1.6之间，而此时数据的平均距离为45.26，因此最佳放缩为 0.03.
- 最终，我所找到的最佳σ取值为：0.03 * 平均距离。
- 对于5000的数据量，平均距离为45.22，σ最佳取值为1.3左右.
...
- 按照交叉验证，使用3600的数据量，在C=10的情况下，得到的Micro F1和σ取值的关系图如下：  
  ![ss](pics/SVM-rel.png)  
  根据关系图，预测最佳sigma取值为 平均距离的0.03倍。

- 对于5000的训练集（平均距离为45.22），按照最佳参数（比例为0.03），C=10，σ=1.3, 得到的最终评估结果如下：  
  ![ss](pics/5000-1.3.png)  
  可见达到了0.59的Accuracy，0.52的MacroF1和0.59的MicroF1。对于这个训练集大小来说，效果是比较好的。

- 通过分析图表和前面的交叉验证可以发现：  
  - 线性核不是很适合本问题。
  - 本问题适合用高斯核来做，且sigma不宜过大。
  - 不同规模的数据，分布相同有着相同的平均距离。
  - 同样的数据量（5000），SVM的效果远好于决策树和KNN。
